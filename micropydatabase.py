...
class Database:
    def __init__(self, database: str, rows_per_page: int, max_rows: int,
        ...
        if not dir_exists(self.path):
            raise Exception("Database not found at " + self.path)
    ...
    def create(database: str, rows_per_page: int = 10, max_rows: int = 10000):
        ...
        if not dir_exists(database):
            ...
            raise Exception('Database \"' + database + '\" is \
    ...
    def open(database: str):
        ...
        if file_exists(schema_path):
            ...
            raise Exception('Database, \"' + database + '\" doesn\'t exist.\
                Please try again.')
...
class Table:
    ...
    def create_table(database, table: str, columns: list,
        ...
        if not dir_exists(table_folder):
            ...
            raise Exception('Table \"' + table + '\" already exists.\
        ...
        if dir_exists(path):
            ...
            for file_name in os.listdir(path):
                if file_name[-4:] in ['temp', 'vacu']:
                    raise Exception("Some temporary data page files are\
                    still in your table.")
            return Table(database, table, definition['columns'],
                         definition['settings']['rows_per_page'],
                         definition['settings']['max_rows'])
        else:
            raise Exception('Table \"' + table + '\" does not exist in\
            database \"' + database.path + '\". Please try again.')
        ...
        if isinstance(data, list):
            ...
            while total > 0:
                ...
                if self.current_row + insert_number > self.max_rows:
                    raise Exception('Table \"' + self.name + '\" \
                    can\'t fit all those rows.')
        ...
        # If not multi-insert
            ...
            if self.__scrub_data(data):
                ...
                if self.current_row < self.max_rows:
                    ...
                    raise Exception('Table \"' + self.name + '\" is full.')
            else:
            ...
            with open(self.path + '/' + f, 'r') as data:
                for line in data:
                    ...
                    if line != '\n':
                        ...
                        for query in queries:
                            if query in cur_data['data'].keys() \
                                    and cur_data['data'][query] in queries[query]:
                                found = True
    ...
    def __check_write_success(self, data, page: str, method: str) -> bool:
        ...
        with open(page, 'r') as f:
            for line in f:
                if current_line == looking_for_line:
                    if method == 'update':
                        if json.loads(line)['row_id'] == row_id \
                                and json.loads(line)['data'] == data[row_id]:
                            return True
                    elif method == 'delete':
                        if line == '\n':
                            return True
        ...
        with open(page, 'r') as f:
            for line in f:
                if len(line) > 1:
                    last_line = line
            if last_line:
                if data["row_id"] == json.loads(last_line)["row_id"]\
                        and data["data"] == json.loads(last_line)["data"]:
                    return True
        return False
    ...
    def __insert_modify_data_file(self, page: str, data: dict = None,
        ...
        if fast:
            ...
            # First we copy the data page file to a temp file
            # if the data page file exists
            temp_path = page + '.temp'
                    ...
                    if current_row_id in update_data:
                        ...
                        # If we are updating a row:
                            ...
                            if current_data['row_id'] == current_row_id:
                                ...
                                raise Exception("Woah we thought " + line +\
                                            " was row_id " + current_row_id + " and almost stomped the wrong row's data!")
    ...
    def __data_file_for_row_id(self, row_id: int):
        ...
        path = self.path + '/data' + str(first_number) +\
            '_' + str(second_number) + '.dat'
        return path
    ...
    def __scrub_data(self, data: dict, fill_missing: bool = True):
        ...
        try:
            ...
            raise Exception('Column \"' + column + '\" doesn\'t exist in the\
                            table \"' + self.name + '\".')
